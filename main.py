...
def sort_patients(
        ...
        order :Optional[str] = Query("asc",description="default is asc mention desc otherwise"),
        db :sqlite3.Connection = Depends(get_db)
    ):
    valid_fields = ['height','weight','bmi']
    if column not in valid_fields:
        raise HTTPException(status_code=400,detail={"invalid":f"invalid field request,select from {valid_fields}"})
    
    if order not in ['asc','desc']:
        raise HTTPException(status_code=400,detail={"invalid":f"invalid order,select from [asc,desc]"})
